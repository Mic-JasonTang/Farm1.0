...
public class UploadPhotoServlet extends HttpServlet {
	...
	private String storePath = ""; // 储存绝对路径，用于存储文件用
	private String relativePath = "";// 存储的相对路径，存放进入数据库
	private final String dir = "files";

	// 将照片信息保存到数据库中
	public boolean save(String productId, String productClass,
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (FileItem fileItem : items) {
				if (fileItem.isFormField()) {
					...
					// 将照片信息保存到数据库
					/**
					 * 将产品id 产品类别 guid文件名， 真实文件名 相对路径保存进入数据库
					 */
					if (save(productId, productClass, guidFileName,
							realFileName, relativePath)) {
		...
		// System.out.println("request.getContextPath()的值是" +
		// request.getContextPath());
		// response.sendRedirect(request.getContextPath() +
		// "/sellerCenterUploadPhotoes.jsp");
	}
	...
	private void processUploadfile(FileItem fileItem) {
		...
		if (realFileName != null) {
			guidFileName = UUID.getUUID() + "_"
					+ FilenameUtils.getName(realFileName);
		}
	...
	System.out.println("存储目录的绝对路径:" + storePath);
			//计算相对路径和文件名
			relativePath = dir + "/" + childDirectory + "/" + guidFileName;
	System.out.println("存储目录的相对路径:" + relativePath);
